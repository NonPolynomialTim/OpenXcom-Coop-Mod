...
- **Tests** (run `python tools/coop_test/<file>` — spawns real game windows).
  All are self-contained: each bootstraps a brand-new campaign, so they need
  no pre-existing save and run on bare OXCE (no external mod required).
  ...
  - `test_geoscape_sync.py` — geoscape-sync A/B validation driver.
  - `test_txq_flood.py` — deterministic TX-queue-full / 19k-ping repro.
  - `test_server_browser.py` — rendezvous combobox (see below).
  - `boot_check.py` — single-instance smoke test for install validation.
- `[coop-transfer]`/`[testserver]` lines land in each instance's `openxcom.log`
  (`%TEMP%\oxc-coop-test\<host|client>-user\`).
...
## Autonomous play driver (removed — was `play_harness.py`)

The autonomous 2-player playthrough driver (`bringup`/`advance`/`play_month`/
`dispatch`/`battle`/`killtest`/`campaign`) was removed from this branch: it
required X-Com Files active (`activate_xcf`, `make_xcf_user_dir`), so it could
not run in isolation on bare OXCE. The geoscape/battlescape **commands** it
drove still exist in `TestServer::execute` (see the command catalog above), so
the driver can be reconstructed against an isolation-friendly mod when needed.
The geoscape/battle gotchas below still apply to anyone writing such a driver.
